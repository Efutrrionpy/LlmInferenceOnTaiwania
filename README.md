...
- `Qwen3.5 397B-A17B MoE GPTQ` loads on 2 nodes / 16 x V100 with `TP=16`, `PP=1`, and reaches `66.71 tok/s` at c=32 using `FLASH_ATTN_V100`.
...
`Llama 3.1 405B GPTQ` is the only complete two-backend comparison. The V100 FlashAttention fork improves the highest-throughput point by about `4.7%` over stock vLLM c=32 (`83.73` vs `79.91 tok/s`) and greatly lowers TTFT. For `Qwen3.5 397B-A17B MoE GPTQ`, `FLASH_ATTN_V100` is the successful backend in this environment.
